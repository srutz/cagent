...
export async function runSettingsWizard(): Promise<void> {
	...
	output.writeln(`${c.dim}No settings.json found — let's create one.${c.reset}\n`);

	// 1. Pick provider
	output.writeln(`${c.bold}Select a provider:${c.reset}`);
	output.writeln(`  ${c.cyan}1${c.reset}) Anthropic (Claude)`);
	output.writeln(`  ${c.cyan}2${c.reset}) OpenAI (GPT-4, etc., compatible with Azure and most LLMs)`);
	output.writeln(`  ${c.cyan}3${c.reset}) Ollama (local)\n`);
	...
	while (!provider) {
		...
		else output.writeln(`${c.red}Invalid choice. Enter 1, 2, or 3.${c.reset}`);
	}
	...
	output.writeln(`${c.dim}Run customagent again to start.${c.reset}\n`);
